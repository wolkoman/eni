...
export function SelfServiceInput<S extends SSType>(props: {
  type?: string,
  input?: 'textarea',
  disabled?: boolean,
  placeholder?: string
  ...
  return <Tag
    ...
    className={`bg-white rounded focus:border-black/50 font-semibold ${props.disabled ? '' : 'px-2 py-1 border border-black/20'} outline-none w-full ${props.input === "textarea" && 'h-36'}`}
  />;
}

export function SelfServiceImageSizeToggle<S extends SSType>(props: {
    disabled?: boolean,
} & SSProps<S>) {
    const sizes = [120,180]
    const currentSize = props.form[0][props.name];
    const nextSize = sizes[((sizes.findIndex(size => size == currentSize) ?? sizes[0]) + 1) % sizes.length]
    return <div
      className="border-black/20 w-12 border border-l-0 rounded grid place-items-center cursor-pointer"
      onClick={() => props.form[1](rest => ({...rest, [props.name]: nextSize}))}
    ><PiResizeBold/></div>;
}

export function SelfServiceEditor<S extends SSType>(props: {
  disabled?: boolean
} & SSProps<S>) {
...
export function SelfServiceFileUpload<S extends SSType>(props: SSProps<S> & { accept?: string }) {
  ...
  >
    ...
    <input id="dropzone-file" type="file" className="hidden" accept={props.accept} onChange={handleFileChange}
           multiple/>
  </label>;
}
...
function FileItem(props: { name: string, link?: string, onDelete?: () => void }) {
  ...
  return props.link
    ...
    }} className="hover:bg-black/5 cursor-pointer"><Content/></div>
