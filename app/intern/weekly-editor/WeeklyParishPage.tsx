...
const componentStyle = "break-inside-avoid text-sm border border-black float-start rounded grow flex flex-col justify-center";

function ArticleComponent({item}: { item: Article }) {
  return <div className={`${componentStyle} px-6 py-4`}>
    <div className="flex justify-between my-2 items-center">
      <div className=" font-bold text-lg ">{item.title}</div>
      <div className="italic" dangerouslySetInnerHTML={{__html: item.author}}/>
    </div>
...
function TeaserComponent({item}: { item: Teaser }) {
  ...
  return <div className={`${componentStyle} text-center px-6 py-4`}>
    <div className="" dangerouslySetInnerHTML={{__html: item.preText}}/>
