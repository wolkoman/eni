...
export function applySuggestion(suggestion: Collections["eventSuggestion"], event?: Omit<CalendarEvent, "id">, markAsSuggestion?: boolean): CalendarEvent {
    ...
    const apply = (diff: Diff[], text: string) => dmp.patch_apply(dmp.patch_make(diff), text);
    const date = apply(suggestion.data.date, event?.date ?? "")[0];
    console.log({suggestion})
    return {
        id: `suggestion_${suggestion._id}`,
        ...(event ?? {tags: []} as any as CalendarEvent),
        summary: apply(suggestion.data.summary, event?.summary ?? "")[0],
        description: apply(suggestion.data.description, event?.description ?? "")[0],
        date: date,
        start: {dateTime: `${date}T${apply(suggestion.data.summary, event?.start ? new Date(event?.start.dateTime).toLocaleTimeString("de-AT", {timeZone: "Europe/Vienna"}) : "")[0]}`},
