...
export type ReaderData = { [eventId: string]: { liturgy: string, cancelledBy?: string[] }  & Record<ReaderRole, ReaderInfo> }
...
export function getTasksFromReaderData<T>(readerData: ReaderData, eventMapper: (eventId: string) => T): ReaderTask<T>[] {
    return Object.entries(readerData)
        ...
        .filter(({event}) => event)
        .flatMap(({event, data}) => (<ReaderRole[]>['reading1', 'reading2', 'communionMinister1', "communionMinister2"]).map(role => (            {
            event,
